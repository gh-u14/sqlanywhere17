...
/** Version 6 introduced a way to set/get autocommit
 *
 * You must define _SACAPI_VERSION as 6 or higher for this functionality.
 */
#define SQLANY_API_VERSION_6		6

/** Version 7 introduced BOOLEAN support
 *
 * You must define _SACAPI_VERSION as 7 or higher for this functionality.
 */
#define SQLANY_API_VERSION_7		7

/** If the command line does not specify which version to build, 
 * then build the latest version. 
 */
#ifndef _SACAPI_VERSION
#define _SACAPI_VERSION	SQLANY_API_VERSION_7
#endif

/** Returns the minimal error buffer size.
 */
#define SACAPI_ERROR_SIZE		512
...
typedef signed long long sacapi_i64;

#if defined(DBCAPI_STRICT)

/** A boolean type for strict type checking */
typedef enum	sacapi_bool {
    sacapi_false    = 0,
    sacapi_true	    = 1
};
...
/** A boolean type that is portable */
typedef sacapi_i32 	sacapi_bool;
#define sacapi_false    0
#define sacapi_true	1
...
typedef void (SQLANY_CALLBACK *SACAPI_CALLBACK_PARM)( a_sqlany_stmt *	stmt,
                                                      const char *      warning_message,
                                                      sacapi_i32        warning_code,
...
typedef struct a_sqlany_column_info
{
    ...
    /// For systems that support both alias name and column name (internal use only).
    char *                      column_name;
#endif
 ...
 * \return sacapi_true on success, sacapi_false otherwise
 * \sa sqlany_fini()
 */
 ...
 * \return sacapi_true if the connection is established successfully or sacapi_false when the connection fails. Use sqlany_error() to
 ...
 * \return sacapi_true when successful or sacapi_false when unsuccessful.
 ...
 * \return sacapi_true on success or sacapi_false on failure.
 */
...
sacapi_bool sqlany_send_param_data( a_sqlany_stmt * sqlany_stmt, sacapi_u32 index, char * buffer, size_t size );
     ...
     *
     * \param sqlany_stmt A valid statement object.
     ...
     *
     * \param sqlany_stmt A valid statement object.
     * \return The number of rows fetched or -1 if a cursor is not open.
 ...
 *
 * \param sqlany_stmt A valid statement object.
 * \return sacapi_true when statement is executed successfully or sacapi_false otherwise.
 * \sa sqlany_prepare()
 */
 ...
 *
 * \param sqlany_stmt A statement object that was executed by sqlany_execute() or sqlany_execute_direct().
 * \param row_num The row number to be fetched. The first row is 1, the last row is -1.
 * \return sacapi_true when the fetch is successful or sacapi_false otherwise.
 ...
 * \return sacapi_true when successfully positioned to the next result set, or sacapi_false otherwise.
 ...
 * \return sacapi_true when successful, or sacapi_false otherwise. A failure
 * can happen if any of the parameters are invalid or if there is not enough
 * memory to retrieve the full value from the SQL Anywhere database server.
 ...
 * \return sacapi_true when successful, or sacapi_false otherwise. Failure occurs when any of the supplied parameters are invalid.
 * \sa sqlany_execute(), sqlany_execute_direct(), sqlany_fetch_absolute(), sqlany_fetch_next(), sqlany_set_rowset_pos()
 */
 ...
 * \return sacapi_true when successful, or sacapi_false when the column index is out of range,
 ...
 * \return sacapi_true when successful, or sacapi_false otherwise.
 * \sa sqlany_commit()
 */
...
#if _SACAPI_VERSION+0 >= SQLANY_API_VERSION_6
/** Sets connection property in memory.
 *
 * This function can be called to set connection properties in name and value pair
 * and connection properties set with this function can then be used to make
 * connection to the database
 *
 * \param sqlany_conn A connection object created by using sqlany_new_connection().
 * \param property The property to be set.
 * \param value The value of the property.
 * \return sacap_true when successful or sacapi_false when unsuccessful.
 */
sacapi_bool sqlany_set_connect_property( a_sqlany_connection * sqlany_conn, const char * property, const char * value );

/** Establishes a connection to a sqlany database server using the connection properties
 * set with sqlany_set_connect_property
 *
 * \param sqlany_conn A connection object created by using sqlany_new_connection().
 * \return sacapi_true when successful or sacapi_false when unsuccessful.
 * \sa sqlany_new_connection(), sqlany_error()
 */
sacapi_bool sqlany_connect2( a_sqlany_connection * sqlany_conn );

/** Sets the AUTOCOMMIT mode to be on or off.
 *
 * The default AUTOCOMMIT mode would be the same as the database setting.
 * This API can be used to change the default setting.
 *
 * When the AUTOCOMMIT mode is set to on, all statements are committed after they are executed,
 * they cannot be rolled back.
 * \param sqlany_conn The connection object on which the commit or rollback operation is to be performed.
 * \param mode A value of 1 or 0.  The value 1 will enable AUTOCOMMIT and the value 0 will disable AUTOCOMMIT
 * \return sacapi_true on success, sacapi_false otherwise.
 * \sa sqlany_commit(), sqlany_rollback()
 */
sacapi_bool sqlany_set_autocommit( a_sqlany_connection * sqlany_conn, sacapi_bool mode );

/** Retrieve the current setting of the AUTOCOMMIT mode.
 *
 * \param sqlany_conn The connection object returned from sqlany_new_connection().
 * \param mode An autocommit mode to be retrieved from the driver.
 * \return sacapi_true on success, sacapi_false on failure which can occur if sqlany_conn is an invalid sqlany connection object.
 */
sacapi_bool sqlany_get_autocommit( a_sqlany_connection * sqlany_conn, sacapi_bool * mode );

/** Sets the transaction isolation level.
 *
 * The default isolation level would be the same as the database setting.
 * This function can be used to change the default setting of the isolation level.
 * \param sqlany_conn The connection object on which the transaction isolation level is to be set.
 * \param isolation_level A value of 0, 1, 2, or 3 that represents READ_UNCOMMITTED, READ_COMMITTED,
 * REPEATABLE_READ, or SERIALIZABLE respectively
 * \return sacapi_true on success, sacapi_false otherwise.
 */
sacapi_bool sqlany_set_transaction_isolation( a_sqlany_connection * sqlany_conn, sacapi_u32 isolation_level );

/** Sets the query timeout of a statement.
 *
 * The default statement query timeout would be the as same as the database setting.  This
 * function can be used to change the default query timeout value.
 * Please note, this setting will change the query time for all the statements created on sqlany_conn
 * \param sqlany_stmt A statement object returned by the successful execution of sqlany_prepare().
 * \param timeout_value A query timeout value in seconds
 * \return sacapi_true on success, sacapi_false otherwise.
 */
sacapi_bool sqlany_set_query_timeout( a_sqlany_stmt * sqlany_stmt, sacapi_i32 timeout_value );

/** The following methods are for internal use only
*/
sacapi_i32 sqlany_get_function_code( a_sqlany_stmt * sqlany_stmt );
sacapi_i32 sqlany_get_param_data( a_sqlany_stmt * sqlany_stmt, sacapi_u32 param_index, size_t offset, void * buffer, size_t size );
sacapi_bool sqlany_register_warning_callback( a_sqlany_connection * sqlany_conn, SACAPI_CALLBACK_PARM callback, void * user_data );
sacapi_bool sqlany_register_message_callback( a_sqlany_connection * sqlany_conn, SACAPI_CALLBACK_PARM callback, void * user_data );
...
/** \example examples\dbcapi_fetchtest.cpp
 * This example shows how to bind columns and perform wide (batch) fetches on multiple result sets.
 */

/** \example examples\dbcapi_isql.cpp
 * This example shows how to write a simple interactive SQL application using dbcapi.
 */
